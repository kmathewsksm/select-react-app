...
describe("SingleSelectComponent", () => {
  ...
  it("renders with placeholder", () => {
    ...
    expect(
      screen.getByText("Select your option here for first select component")
    ).toBeInTheDocument();
  });
  ...
  it("handles isMenuOpen prop correctly", () => {
    const { rerender } = render(
      <SingleSelectComponent onChange={onChangeMock} isMenuOpen />
    );
    expect(screen.getByText("Chocolate")).toBeVisible();
    expect(screen.getByText("Strawberry")).toBeVisible();
    ...
    rerender(
      <SingleSelectComponent onChange={onChangeMock} isMenuOpen={false} />
    );
